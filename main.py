...
class Action(ABC):
    ...
    def serialize(self) -> dict:
        ...
        for (k, v) in self.__dict__.items():
            if out.get(k) is None and not callable(v):
                out.update([(k,v)])
...
class Delete(Action):
    ...
    def execute(self, src: str):
        if self.validate_func is None:
            print(f"deleted '{src}' without confirmaton")
            return
        
        if self.validate_func():
            print(f"deleted '{src}' after confirmation")

        else:
            print("validation failed, not deleting")            
...
a = Copy("dfs").get_destination()
d = Delete(lambda : True).serialize()
print(d)
# d.execute("")
# print(a)
# token = Token("fadf", d)
